...
function terapkanDataKeTemplate() {
  ...
  if (dataKado.link_foto_1) {
    screens.opening.style.background = `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('${dataKado.link_foto_1}') center/cover no-repeat`;
  }
  if (dataKado.link_foto_2) {
    document.body.style.background = `url('${dataKado.link_foto_2}') center/cover no-repeat`;
  }
  
  // Musik
  if (dataKado.link_musik) {
    bgm.src = dataKado.link_musik;
  }
